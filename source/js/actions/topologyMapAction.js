...
export const getTopologyData = (axiosdata, typeId, props) => {
    ...
    return function (dispatch) {
        ...
        //var response_UIV= require('../helpers/ServerTrail_with_level_order.json');//without device details
        var response_UIV = require('../helpers/serverTrail_with_depiction2.json'); //without device details and depiction level
        ...
        if (UIVDirectory) {
            ...
            Object.keys(response_UIV.diagraphModel.edges).forEach((item, index) => {
                ...
                var level='';
                if (response_UIV.diagraphModel.edges[item].relations[0] == 'USES' && response_UIV.diagraphModel.edges[item].direction == 'OUTGOING') {
                    ...
                    if(response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].from]._type=='cssLink' || response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].from]._type=='cssConnection' ){
                    ...
                    UIVDirectory[response_UIV.diagraphModel.edges[item].from] = {
                        ...
                        detail: response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].from].attributes,
                        layer: level,
                    ...
                    if(response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].to]._type=='cssPort' || response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].to]._type=='cssPhysicalComponent'){
                        UIVDirectory[response_UIV.diagraphModel.edges[item].to] = {
                            type: "Equipment",
                            name: response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].to].attributes['globalName'],
                            id: response_UIV.diagraphModel.edges[item].to,
                            terminations: [],
                            ePoints: [],
                            state: {},
                            cNodes: [],
                            detail: response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].to].attributes,
                            layer: level,
                    ...
                    if (!UIVDirectory[response_UIV.diagraphModel.edges[item].from]) {
                        console.log("inside 'from' relation:", response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].from]._type, ' :: ', response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].to].attributes['id'], 'name ::', response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].to].attributes['globalName']);
                    ...
                    if (!UIVDirectory[response_UIV.diagraphModel.edges[item].to]) {
                        console.log("inside 'to' relation", response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].to]._type, ' :: ', response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].to].attributes['id'], 'name ::', response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].to].attributes['globalName']);

                        level=''; Object.keys(response_UIV.diagraphView.depictionLevel).forEach((item2, innerIndex) => { if(response_UIV.diagraphView.depictionLevel[item2].indexOf(response_UIV.diagraphModel.edges[item].to)!=-1){level= innerIndex}});

                        if (response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].to]._type == "cssLink" || response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].to]._type == "cssConnection") {
                            ...
                            UIVDirectory[response_UIV.diagraphModel.edges[item].to] = {
                                ...
                                ePoints: [],
                                state: {},
                                cNodes: response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].to].child,
                                detail: response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].to].attributes,
                                layer: level,
                                _type: response_UIV.diagraphModel.nodes[response_UIV.diagraphModel.edges[item].to]._type
                            }
...
console.log("terminations for::",UIVDirectory[item]," terminations: ",UIVDirectory[item].terminations);
        ...
        */
        ...
        console.log(response_UIV.diagraphView.depictionLevel[1]);
...
//check for cross connects:
        ...
        dispatch({
            ...
            payload: {
                rootPathID: rootPathIdValue,
                data: UIVDirectory,
                customHookData: customHookObject
            }
        });
        //  });
        // } catch (err) {
...
function MapNode_ForMultipleLocations(parentObject, directory, typeId, EPData) {
    ...
    var lStateNodes = hasAssociation.filter((x) => {
        var item = x.Target ? x.Target : x.Source;
        return item["@class"].toLowerCase() == "state"
    })
...
function GetLastLocation(object, directory, lEndPointsTypes) {
    ...
    var lCNodesToProcess = hasAssociation.filter((x) => {
        var item = x.Target ? x.Target : x.Source;
        return lEndPointsTypes.findIndex((x) => x.toLowerCase() == item["@class"].toLowerCase()) != -1
    })
    if (lCNodesToProcess.length > 0) {
...
function GetMultipleEntity(object, lCNodesToProcess, lEndpoints, lEndPointsTypes, location, lStates, directory, typeId) {
    ...
    this.cNodes = (function (object) {
        if (lCNodesToProcess && lCNodesToProcess.length > 0) {
            ...
            var filteredNodes = lCNodes.filter((cNode) => {
...
function MapNode(object, directory) {
    ...
    var lCNodesToProcess = hasAssociation.filter((x) => {
        var item = x.Target ? x.Target : x.Source;
        return item["@class"].toLowerCase() == "path"
    })
    var location = hasAssociation.filter((x) => {
        var item = x.Target ? x.Target : x.Source;
        return item["@class"].toLowerCase() == "location"
    })

    if (object["@class"].toLowerCase() == "path" && hasAssociation.length > 1) {
        var lEndpoints = hasAssociation.filter((x) => {
            var item = x.Target ? x.Target : x.Source;
            return lEndPointsTypes.indexOf(item["@class"].toLowerCase()) != -1
        })
        if (!lEndpoints || lEndpoints.length == 0) {
...
function sortNode(nodeID, directory) {
    ...
    if (lcNodes && lcNodes.length > 0) {
        ...
        directory[nodeID].cNodes = lSortedNodes.map((sNode) => {
            sortNode(sNode.id, directory);
            return sNode.id
        })
    }
}